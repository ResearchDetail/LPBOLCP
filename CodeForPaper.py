...
def process(db_table_name, repeat):
    ...
    for cou in baifenbi:
        for i in range(repeat):
            ...
            edge = random.sample(net.edges(core),wanted)
            for i in range(len(edge)):
            ...
            for i in xrange(len(net.nodes(core))):  # 遍历所有节点列表
                node1 = net.nodes(core)[i]  # seed node A
                for j in xrange(i, len(net.nodes(core))):
                    node2 = net.nodes(core)[j]  # seed node B
                    if i == j:
                        continue
